...
var commentsList = make([]Comment, 0)
...
func handleCommentsPage(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: actually fetch comments from db

	// Add a new comment to the in memory slice of comments
	commentsList = append(commentsList, Comment{Author: r.FormValue("author"), Text: r.FormValue("text")})

	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("Cache-Control", "no-cache")
	err = json.NewEncoder(w).Encode(commentsList)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func handleCommentsGet(r *http.Request) interface{} {
	commentMutex.Lock()
	defer commentMutex.Unlock()

	copyCommentsList := make([]Comment, len(commentsList))
	copy(copyCommentsList, commentsList)
